...
# TODO: this file is copied directly from papis/testing.py and black-ed
# TODO: need to remove when we depend on papis 0.14
...
def create_random_file(
    ...
    :arg cwd: a base directory passed to :func:`tempfile.NamedTemporaryFile`.
    """
    if filetype is None:
    ...
    with tempfile.NamedTemporaryFile(
        dir=cwd, suffix=suffix, prefix=prefix, delete=False
    ) as fd:
        fd.write(buf)
...
def populate_library(libdir: str) -> None:
    ...
    for i, data in enumerate(PAPIS_TEST_DOCUMENTS):
        ...
        if num_test_files:
            doc_data["files"] = [
                os.path.basename(create_random_file(cwd=folder_path))
...
class TemporaryConfiguration:
    ...
    def create_random_file(
        ...
        return create_random_file(
            filetype, suffix=suffix, prefix=prefix, cwd=self.tmpdir
        )

    def __enter__(self) -> "TemporaryConfiguration":
        if self._tmpdir is not None:
            raise ValueError(f"{type(self).__name__!r} cannot be nested")
...
class TemporaryLibrary(TemporaryConfiguration):
    ...
    def __enter__(self) -> "TemporaryLibrary":
        ...
        if self.use_git:
            ...
            run(["git", "config", "user.name", "papis"], cwd=self.libdir)
            run(["git", "config", "user.email", "papis@example.com"], cwd=self.libdir)
...
class ResourceCache:
    ...
    * ``"both"``: both local and remote resources are updated.
    ...
    def get_local_resource(
        self, filename: str, data: Any, *, force: bool = False
    ) -> Any:
